...
class LogisticRegression_ADMM(linear_model.LogisticRegression):
    ...
    def cal_theta(self,theta,X,y):
        ...
        res = optimize.minimize(J,theta,method=self.solver,jac=grad_J,hess=hess_J,options=optionFlag,args=(X,y))
        return res.x
...
class LogisticRegression(linear_model.LogisticRegression):
    ...
    def cal_theta(self,theta,X,y,u_z=None,l1=False):
        ...
        def calp(theta,X):
            ...
            for n in range(self.n_samples):
                ...
                for k in range(self.K):
                    Y[n, k] = np.exp(np.dot(theta[:,k], X[n,:])) / denominator
            return Y
        ...
        def grad_J(theta,X,T,p):
            g=0
            if l1:g= self.rho*(theta.T+u_z.T).reshape(-1)
            grad=np.array([np.dot(X.T, p[:,i] - T[:,i]) for i in range(self.K)]).reshape(-1)
            return grad+g
                ...
                for m in range(self.K):
                    for n in range(self.n_samples):
                        if m==k:
                            ...
                            H[m+k*self.K] += temp * X[n].reshape(self.n_features,1) * X[n].reshape(1,self.n_features)  # 縦ベクトルx横ベクトル
            hess=np.zeros(self.n_features*self.K*self.n_features*self.K).reshape(self.n_features*self.K,self.n_features*self.K)
            for i in range(self.K):
                hess[i*self.n_features:(i+1)*self.n_features,i*self.n_features:(i+1)*self.n_features]=H[i+i*self.K]
            g=0
            if l1:g=np.diag([self.rho for i in range(self.n_features*self.K)])
            return hess+g
        ...
        for iter_ in range(self.iter_max):
            ...
            theta_new  = theta - np.dot(np.linalg.inv(hess),grad).reshape(self.K,-1).T

            if iter_!=0:
                diff = np.linalg.norm(theta_new - theta) / np.linalg.norm(theta)
                if diff < 0.01: break
            theta = theta_new
        if self.verbose:print "iter: ",iter_,
        return theta.reshape(-1,self.K)

    def fit(self,X,y):
        self.classes_=(np.unique(y))
        X_=np.hstack((np.ones((X.shape[0], 1.)), X)) #inser intercept line
        y_=np.array([[1 if i==j else 0 for i in y] for j in self.classes_[:]]).T
        self.n_samples ,self.n_features = X_.shape
    ...
    def fit_normal(self,X,y):
        ...
        if theta.shape[0]==2:theta=theta[np.newaxis,1]
        self.coef_=theta[:,1:]
    ...
    def fit_l1(self,X,y):
        ...
        for iter_a in range(self.iter_admm):
            ...
            if self.verbose:
                ...
                z_save= z.T
                if z_save.shape[0]==2:z_save=z_save[np.newaxis,1]
                self.coef_=z_save[:,1:]
    ...
    def fit_l1_group(self,X,y):
        ...
        z= z.T
        if z.shape[0]==2:z=z[np.newaxis,1]
        self.coef_=z[:,1:]
        self.intercept_=z[:,0]
    ...
    def plot(self,X,y,W_t,filename):
        ...
        if x2[0]=="inf":ff(W_t,0,1,color="red")
        else: plt.plot(x1, x2, 'r-')


        x1 = np.linspace(-20, 20, 1000)
        x2 = [f(x, W_t, 1, 2) for x in x1]
        if x2[0]=="inf":ff(W_t,1,2,color="blue")
        else:plt.plot(x1, x2, 'b-')

        x1 = np.linspace(-20, 20, 1000)
        x2 = [f(x, W_t, 2, 0) for x in x1]
        if x2[0]=="inf":ff(W_t,2,0,color="green")
        else:plt.plot(x1, x2, 'g-')
