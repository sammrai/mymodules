...
class GroupLasso(linear_model.LinearRegression):    
    ...
    def fit(self, A, b,group=None):
        ...
        if group==None:group=[i for i in range(A.shape[1])]
        subgroup=  np.array([np.array(range(len(group)))[np.array(group)==i] for i in np.unique(group)])
        ...
        for iter_ in range(self.iter_num):
            ...
            for sub_g in subgroup:
                z[sub_g] = self.GroupSoftMax(self.alpha/self.rho,(x+u)[sub_g])
