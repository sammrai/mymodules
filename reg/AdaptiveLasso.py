...
class AdaptiveLasso(linear_model.LinearRegression):    
    ...
    def fit_one(self, X, y):
        ...
        self.intercept_=np.average(y_)-np.dot(np.mean(X,axis=0),self.coef_)
    ...
    def fit(self, X, y, fold_size=5):
        if not self.gamma: gammas=[0.5, 1., 2.]
        else: gammas=np.expand_dims(self.gamma, axis=0)
        if not self.alpha: alphas=self.setalpha(X,y)
        else: alphas=np.expand_dims(self.alpha, axis=0)
        ...
        if len(gammas)==1 and len(alphas)==1:
            ...
            return 0
        # else:
            # if self.verbose: print "### MESSAGE ### : calling num of function fit_one will be %d"%len(gammas)*len(alphas)

        alp_store=[]
        for one_g in gammas:
            for one_a in alphas:
                alp_store.append([self.cvscore(X,y,one_g,one_a,fold_size=fold_size),one_g,one_a])
    ...
    def fit_(self,X,y,target,K=10,iter_=3):
        def process(one_a):
            clf_=AdaptiveLasso(alpha=one_a,gamma=self.gamma,verbose=self.verbose)
            clf_.fit(X,y)
        ...
        def getPinchindex(lis,target):
            ...
            if lis[0]==target:
                return 0,1
            raise ValueError("#ERROR : target value is not exist in list. \nlis:%s target:%s"%(lis,target))
        ...
        for i in range(iter_):
            if i==0: alphas=self.setalpha(X,y,K=K)
            else: alphas=np.logspace(np.log10(alphas[pinch[0]]) , np.log10(alphas[pinch[1]]) , num=K)
            results = pool.map(process, alphas)
            pinch=getPinchindex(results,target)

            if self.verbose:print target,results,pinch
