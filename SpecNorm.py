...
parser.add_argument('-o', '--out-suffix', metavar='SUFFIX', type=str, default='norm')
parser.add_argument('-w', '--wavelength', metavar='WAVELENGTH', type=float, required=False, default=None)
parser.add_argument('-wx', '--wave-max', metavar='MAX_WAVELENGTH', type=float, required=False, default=None)
parser.add_argument('-wm', '--wave-min', metavar='MIN_WAVELENGTH', type=float, required=False, default=None)
parser.add_argument('-e', '--energy', action='store_true', required=False, default=False)
parser.add_argument('-s', '--surface', action='store_true', required=False, default=False)
...
for wl in range(wl_min, wl_max):
    ...
    for i,spec in enumerate(spec_in):
        ...
        norm[i] = spec_n
        err += np.linalg.norm(spec_n[wl_min:wl_max+1,1]-best_spec[wl_min:wl_max+1,1])
