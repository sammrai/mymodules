...
def getparse():
    ...
    parser.add_argument('-logy', '--log_y', action='store_true', required=False, default=False)
    parser.add_argument('-logx', '--log_x', action='store_true', required=False, default=False)
    ...
    parser.add_argument('-g', '--ggplot', action='store_true', required=False, default=False,help='Use seaborn')
    parser.add_argument('-b', '--bar', action='store_true', required=False, default=False,help='bar graph')
    ...
    parser.add_argument('-ds', '--datasize', metavar='DATA_SIZE', type=int, required=False, default=None ,help="Specify the number of plot data.")
...
#plot index
if args.datasize==None:args.datasize=len(spec_in)
plotlist=np.linspace(0,len(spec_in),args.datasize, dtype = int, endpoint=False)
...
for i, spec in enumerate(spec_in):
    ...
    try:
        spec[:,0],spec[:,1]=[spec[j,0] for j in np.argsort(spec[:,0])],[spec[j,1] for j in np.argsort(spec[:,0])]
    except:
        pass
    ...
    if args.fill:
        ...
        try:
            if spec.shape[0]<spec.shape[1]:spec=spec.T
            plt.plot(spec[:,0],spec[:,1], args.line_type,linewidth=line, color=line_color, label=label_list[i],markersize=args.marker_size)
        except:
            try:
                plt.plot(spec, args.line_type,linewidth=line, color=line_color, label=label_list[i],markersize=args.marker_size)
            except:
                plt.plot(spec.T[0],spec.T[1], args.line_type,linewidth=line, color=line_color, label=label_list,markersize=args.marker_size)
