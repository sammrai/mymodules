...
def savefig(filename, img,max=None,min=None,color=None,message="",colormap='rainbow'):
    ...
    """    
    img=np.array(img)
    img=np.array([0 if "nan"==str(i) else i for i in list(img.reshape(-1))  ]).reshape(img.shape)

    if filename.split(".")[-1] == "pdf":
        print "#ERROR : .%s format is not supported."%filename.split(".")[-1]
        exit()
    if min is None: min=np.min(img)
    if max is None: max=np.max(img-min)
    if img.ndim !=2:
        print "#ERROR : dimension number is invarid."
        exit()
    if color is None:
        img-=min
        img/=max
...
class socio:
    ...
    def getBandArray(self, band):
        ...
        file_float.seek(band*self.samples*self.bpp, 1)  # offset of each band
    ...
    def getRaw(self):
        ...
        for j in range(self.bands):
            ...
            sys.stderr.write('\r\033[K' + get_progressbar_str(progress,message="getArray"))
            sys.stderr.flush()
    ...
    def getLineArray(self, line):
        ...
        file_float.seek(self.header_offset, 0)  # return to top of data
        file_float.seek(self.bands*self.samples*self.bpp, 1)  # offset of each band
    ...
    def __getitem__(self, key):
        ...
        for j in lines:
            for i in samples:
                # print    len(self.sample_cache), len( self.band_cache ) ,len( self.line_cache)
                A.append(self.getSpectrumAt((i,j)))
                progress = (len(self.sample_cache))/float(len(samples))
                sys.stderr.write('\r\033[K' + get_progressbar_str(progress,message="getSpectrumAt"))
                sys.stderr.flush()
        print 
        return np.array(A).reshape(len(lines),len(samples),-1)[:,:,key[2]]
        # return np.array([self.getSpectrumAt((j,i)) for i in samples for j in lines]).reshape(len(lines),len(samples),-1) 
