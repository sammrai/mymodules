...
def golay(*args,**kwargs):
    ...
    if len(args)==3:
        ...
        raise Exception("golay() takes at least 2 arguments (%d given)"%len(args))
    ...
    #Check data
    if len(x) != len(y):
        raise ValueError("Input sequence length is invalid %d %d"%(len(x),len(y)))
    if len(x)-m*2<=3:
        raise ValueError("Input sequence length is too small. %d"%len(x_))
    if m<(dim+1)/2:
        raise ValueError("Smoothness value :m takes at most %d"%((dim+1)/2))
    if m==0 and dim==0:
        return y
    ...
    def diff(_x,_y,_dim,_m):
        ...
        func=lambda s: (lambda m:m(m)(s))(lambda proc: (lambda n: 1 if n == 0 else n * proc(proc)(n-1)))
        seeknum = len(_y)-2*_m
        AA=[np.dot(B, _y[seek:seek+1+_m*2])*func(dim)/dx**_dim for seek in range(seeknum)]
