...
import os
basefile=sys.argv[1]
if os.path.exists(basefile.split(".")[0]): pass
else: os.mkdir(basefile.split(".")[0])

#load raw and mask image
img_ = cv2.imread(sys.argv[1])
try: mask = cv2.imread(sys.argv[2])
except: mask = np.zeros(img_.shape,img_.dtype)
...
def drawedge(img,cordinates,j,color=[0,255,0]):
    ...
    for i in cord:
        img_result[i]=color
    cv2.putText(img_result,"%d"%j,(np.min(cordinates[:,0]),np.min(cordinates[:,1])),font, 1,(255,255,255),2)
    cv2.putText(img_result,"%d"%j,(np.min(cordinates[:,0]),np.min(cordinates[:,1])),font, 1,(0,0,0),1)
    return img_result
