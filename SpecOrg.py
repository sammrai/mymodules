...
parser.add_argument('-o', '--out-suffix', metavar='SUFFIX', type=str, default='org')
parser.add_argument('-av', '--average', action='store_true', required=False, default=False)
parser.add_argument('-sl', '--slice', type=str,  required=False)
parser.add_argument('-w', '--wav', dest='file_wav', default=None, metavar='wavelength')
...
if args.average:
    spec_out[:,1]=np.sum([spec_out[:,1]+i[:,1] for i in spec_in],axis=0)/len(spec_in)

    # save
    file_out= st.mkdir_suff(args.out_suffix,SUFFIX,base=args.file_in[0],ex=".txt")
