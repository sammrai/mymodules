...
parser.add_argument('-o', '--out', type=str, default="img.png", help='Specify output (ex. spec.png spec.txt DIR/out.png)')
parser.add_argument('-s', '--skip', type=int, default='1', help='Reduce output by skipping')
parser.add_argument('-r', '--raw', action='store_true', default=False, help='Write raw image')
parser.add_argument('-b', '--band', nargs='+', default=None, help='Bands to write (ex. -b 800 900)')
parser.add_argument('-l', metavar='FLOAT')
parser.add_argument('-w', dest='wavelength')
parser.add_argument('-a', '--absorbance', action='store_true', required=False, default=False)
parser.add_argument('-t', '--transmittance', action='store_true', required=False, default=False)
parser.add_argument('-i','--interactivemode',action='store_true', required=False, default=False,help='interactivemode')

parser.add_argument('-c', '--PseudoColor', action='store_true', required=False, default=False)
...
parser.add_argument('-d1', '--div1', metavar='FIRST_DERIVATIVE', type=int, required=False, nargs='?',default=0)
parser.add_argument('-d2', '--div2', metavar='SECOND_DERIVATIVE', type=int, required=False, nargs='?',default=0)
parser.add_argument('-sl', '--slice', type=str,  required=False,default=":,:,:")
...
while True:
    ...
    if args.ymax is not None:
            maxx=args.ymax
    if args.interactivemode:
        minn=check(minn,message="enter MIN value [ s: skip , ctl+c : exit] > ")
        maxx=check(maxx,message="enter MAX value [ s: skip , ctl+c : exit] > ")
    ...
    if (args.movie) is not None:
        ...
        os.system("SocMakeMovie.py %s -f 10 -o %s"%(fname_out_base+"*"+suffix,args.movie))
