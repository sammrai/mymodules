...
parser.add_argument('-o', '--out-suffix', metavar='SUFFIX', type=str, default='conv')
parser.add_argument('-m', '--multiply', metavar='MULTIPLY', type=float, required=False, default=1.0)
parser.add_argument('-mv', '--avr-num', metavar='AVERAGE', type=int, required=False, default=1)
parser.add_argument('-sm', '--smooth', metavar='KERNEL_SIZE', type=float, required=False, default=1.0)
parser.add_argument('-ks', '--kernel_size', metavar='KERNEL_SIZE_GAUSS', type=int, required=False, nargs='?',default=10)
parser.add_argument('-pca', '--PCA', action='store_true', required=False, default=False)


parser.add_argument('-l', '--log', action='store_true', required=False, default=False)
parser.add_argument('-t', '--transpose', action='store_true', required=False, default=False)
parser.add_argument('-d0', '--div0', metavar='ZERO_DERIVATIVE', type=int, required=False, nargs='?',default=0)
...
parser.add_argument('-i', '--invert', action='store_true', required=False, default=False)

parser.add_argument('-d', '--dark', metavar='DARK_SPECTRUM', required=False, default=None)
...
# save
    file_out= st.mkdir_suff(args.out_suffix,"conv",base=args.file_in[i],ex=".txt")
    np.savetxt(file_out, spec_out, fmt='%12.8f')
    print 'Saved to ' + file_out
